...
let content = fs.readFileSync('data/lessons.js', 'utf8');
const dataPrefix = "export const lessons = ";
let jsonStr = content.replace(dataPrefix, '').replace(/;$/, '');
let lessonsData = JSON.parse(jsonStr);

updated.forEach(upd => {
  let idx = lessonsData.findIndex(l => l.id === upd.id);
  if(idx !== -1) {
    lessonsData[idx] = { ...lessonsData[idx], ...upd };
  }
});

fs.writeFileSync('data/lessons.js', dataPrefix + JSON.stringify(lessonsData, null, 2) + ';\n');
console.log('Updated 3 lessons');
